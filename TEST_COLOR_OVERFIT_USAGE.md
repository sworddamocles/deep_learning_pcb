...
脚本会把原图和对比图按 batch 拼成可视化大图，便于快速人工检查模型在颜色扰动下的检测稳定性。
...
```bash
python test_color_overfit.py --model runs/train/yolo11_cbam_SimAM_color_robust_1024/weights/best.pt --source D:/deep_learning/ultralytics-8.3.163/dataset_split/images/val --imgsz 1024 --conf 0.25 --iou 0.5 --max-images 300 --batch-size 20 --compare-variant worst --device 0
```
...
- --batch-size: 每批处理多少张图像，到达后立即输出一张批次拼图和批次统计
- --compare-variant: 对比面板显示的扰动类型，默认 worst 表示每张图自动选最不稳定的扰动
- --viz-height: 拼图中每个样本块的目标高度
...
- color_overfit_batch_XXX.jpg: 每个 batch 的原图 vs 对比图拼图
...
脚本里的 match_ratio、conf_ratio、count_ratio 分别衡量“框是否还对得上”“置信度是否稳定”“检测数量是否稳定”。综合分数越高，表示模型对颜色变化越不敏感。批次拼图则用于人工复核：左边是原图检测，右边是对比扰动检测，方便直接看出模型在哪些样本上开始失稳。
